...
!=======================================================================
!                                                                      !
!  Initialize ocean and cice models coupling stream.
!                                                                      !
!=======================================================================
!
      USE mod_param
...
!
      CALL MCTWorld_init (Nmodels, MPI_COMM_WORLD, OCN_COMM_WORLD, OCNid)
!
...
!
      ...
      CALL AttrVect_init (wav2ocn_AV, rList=TRIM(ExportList(Iwaves)), lsize=Asize)
!
!  Initialize attribute vector holding the export data code string of
!  the ocean model.
!
      CALL AttrVect_init (ocn2wav_AV, rList=TRIM(ExportList(Iocean)), lsize=Asize)
...
#include "tile.h"
!
      CALL ocn2cice_coupling_tile (ng, tile, LBi, UBi, LBj, UBj)
...
!***********************************************************************
      SUBROUTINE ocn2cice_coupling_tile (ng, tile, LBi, UBi, LBj, UBj)
...
!
!-----------------------------------------------------------------------
!  Export fields from ocean (ROMS) to cice model.
!-----------------------------------------------------------------------
!
...
!
      IF (Iexport.gt.0) THEN
        ...
        CALL MCT_Send (ocn2wav_AV, ROMStoCICE, MyError)
...
!
 ...
 40   FORMAT (16x,'- ',a,a,                                             &
     &        /,19x,'(Min= ',1p,e15.8,0p,' Max= ',1p,e15.8,0p,')')
      ...
      RETURN
      END SUBROUTINE ocn2cice_coupling_tile
