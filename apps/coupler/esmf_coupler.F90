PROGRAM esmf_coupler
    ...
    USE CICE_ComponentMod, ONLY : CICE_SetServices
    !
    implicit none
    !
    ...
    CALL ESMF_Initialize (defaultLogType=ESMF_LOG_SINGLE,             &
        &                      vm=MyVM,                                    &
        &                      rc=status)
    IF (CheckError(status, 'ESMF', 'esmf_coupler',                  &
    ...
    DO model=1,Nmodels
        ...
        ELSE IF (model.eq.Iice) THEN
            GridName='CICE Model Component'
        END IF
    ...
    !
    !  CICE model registration
    !
    CALL ESMF_GridCompSetServices (GridComp(Iice),                  &
        &                               CICE_SetServices,                &
        &                               status)
    IF (CheckError(status, 'ESMF', 'esmf_coupler',                  &
        &               'setting cice model services')) THEN
        CALL abort (status)
    END IF
    ...
    DO model=1,Nmodels
        ...
        ELSE IF (model.eq.Iice) THEN
            ExpName='CICE Model Component Export State'
            ImpName='CICE Model Component Import State'
        END IF
