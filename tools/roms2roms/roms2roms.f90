...
! Read in temperature
   ...
   do nn=1,Ni
      WHERE (rmaski < 1) temp_in(:,:,nn) = undef
      CALL fill(Lp,Mp,1,Lp,1,Mp,temp_in(:,:,nn),tx,critx,cor,mxs,work,error,nvalue)
   end do
   !check for Kelvin...
...
! Read in salinity
   ...
   do nn=1,Ni
      WHERE (rmaski < 1) salt_in(:,:,nn) = undef
      CALL fill(Lp,Mp,1,Lp,1,Mp,salt_in(:,:,nn),tx,critx,cor,mxs,work,error,nvalue)
   end do
!
! Vertical interpolation
...
! 
  rmaskuv = rmaski
  do nn=1,Ni
     where ( u_in(:,:,nn) < -20 .or. u_in(:,:,nn) > 20 ) rmaskuv = 0
     where ( u_in(:,:,nn) < -20 .or. u_in(:,:,nn) > 20 ) u_in(:,:,nn) = undef
...
!
  rmaskuv = rmaski
  do nn=1,Ni
     where ( v_in(:,:,nn) < -20 .or. v_in(:,:,nn) > 20 ) rmaskuv = 0
     where ( v_in(:,:,nn) < -20 .or. v_in(:,:,nn) > 20 ) v_in(:,:,nn) = undef
