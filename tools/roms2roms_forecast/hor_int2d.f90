subroutine hor_int2d(array_in,Mpi,Lpi,xp,yp,ylon,dx,lon_rhoo,lat_rhoo,Mpo,Lpo,array_out,undef,rmaski,rmasko,tx,critx,cor,mxs)
! ---------------------------------------------------------------------
...
implicit none
  integer :: i,j,k,Mpo,Lpo,margin,Lpi,Mpi
  ...
  real  :: array_in(Lpi,Mpi)
  real, intent(out)  :: array_out(Lpo,Mpo)
...
! Horizontal interpolation
   ...
   DO i=1,Lpo
      ...
      rxm = 1.0-rx
      rym = 1.0-ry
         array_out(i,j) = rxm*rym*array_in(ia,ja)+rx*rym*array_in(ia+1,ja)&
         +rx*ry*array_in(ia+1,ja+1)+rxm*ry*array_in(ia,ja+1)
   ENDDO
   ENDDO
