...
!
! 09.02.2012 - nilsmk@met.no: Added dims for u and v (for detiding)
!
! ----------------------------------------------------------------------
...
program tidenc2roms
 ...
 character(len=5), dimension(Nconstit), parameter :: constit =   &
  ...
  integer :: dimid
  integer :: imax_id, jmax_id, xiu_id, xiv_id, etau_id, etav_id
  ...
  !write (*,'(/,a6,i5,x,a6,i5)') "imax: ", imax, "jmax: ",jmax
  ...
  allocate(indx(nc))
! Very bad hardcoding... verify-algoritmen klarer ikke aa skille noen konstituenter
  indx(9)=13
  indx(10)=11
  indx(11)=12
  do k = 1, nc 
...
!!$     if (verify(trim(constit(13)),trim(str)).eq.0) indx(13)=k
  ...
  status = nf_def_dim(ncid, "xi_u", imax+1, xiu_id); call check_err(status)
  status = nf_def_dim(ncid, "eta_u", jmax+2, etau_id); call check_err(status)
  status = nf_def_dim(ncid, "xi_v", imax+2, xiv_id); call check_err(status)
  status = nf_def_dim(ncid, "eta_v", jmax+1, etav_id); call check_err(status)
  ...
  do k = 1, Nconstit
     ...
     !write (*,'(/,a6,i5,x,a6,i5)') "imax: ", imax, "jmax: ",jmax
     call shiftphase(tide_Eamp,tide_Ephase,uamp,uphase,vamp,vphase,imax,jmax,constit(k),reflat)
     ...
     status = nf_put_vara_real(ncid, varid, (/ 1, 1, k /), (/ imax+2, jmax+2, 1 /), tide_Eamp(0:imax+1,0:jmax+1))
     call check_err(status)
     status = nf_inq_varid(ncid, "tide_Ephase", varid)
     call check_err(status)
     status = nf_put_vara_real(ncid, varid, (/ 1, 1, k /), (/ imax+2, jmax+2, 1 /), tide_Ephase(0:imax+1,0:jmax+1))
     call check_err(status)
     status = nf_inq_varid(ncid, "tide_Cmax", varid)
     call check_err(status)
     status = nf_put_vara_real(ncid, varid, (/ 1, 1, k /), (/ imax+2, jmax+2, 1 /), tide_Cmax(0:imax+1,0:jmax+1))
     call check_err(status)
     status = nf_inq_varid(ncid, "tide_Cmin", varid)
     call check_err(status)
     status = nf_put_vara_real(ncid, varid, (/ 1, 1, k /), (/ imax+2, jmax+2, 1 /), tide_Cmin(0:imax+1,0:jmax+1))
     call check_err(status)

     status = nf_inq_varid(ncid, "tide_Cangle", varid)
     call check_err(status)
     status = nf_put_vara_real(ncid, varid, (/ 1, 1, k /), (/ imax+2, jmax+2, 1 /), tide_Cangle(0:imax+1,0:jmax+1))
     call check_err(status)
     status = nf_inq_varid(ncid, "tide_Cphase", varid)
     call check_err(status)
     status = nf_put_vara_real(ncid, varid, (/ 1, 1, k /), (/ imax+2, jmax+2, 1 /), tide_Cphase(0:imax+1,0:jmax+1))
     call check_err(status)
