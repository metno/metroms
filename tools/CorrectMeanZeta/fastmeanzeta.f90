program fastmeanzeta
! nilsmk@met.no 17/10/2012

use netcdf

implicit none
REAL, DIMENSION(:,:,:), ALLOCATABLE :: zeta,zetamean,zeta_detide,zeta_tide
real, dimension(:), allocatable :: zetameanval
...
integer   :: statusi,statuso,zetaDTVarId,zetaTVarId,zetaVarId2
...
if (statusi /= nf90_noerr) statusi = nf90_inq_dimid(ncid,'clim_time',dim_time)
statusi = nf90_Inquire_Dimension(ncid,dim_time,time_dimname,TIME)

write(*,*) 'infile= ',infile
write(*,*) 'X, Y, TIME = ',X,Y,TIME
allocate(zeta(X,Y,TIME))
allocate(zetameanval(TIME))
statusi = nf90_inq_varid(ncid,'zeta_detided',zetaDTVarId)
!if (statusi /= nf90_noerr) statusi = nf90_inq_varid(ncid,'zeta',zetaDTVarId)
statusi = nf90_get_var(ncid,zetaDTVarId,zeta_detide)
statusi = nf90_inq_varid(ncid,'zeta',zetaTVarId)
statusi = nf90_get_var(ncid,zetaTVarId,zeta_tide)
...
write(*,*) 'X, Y = ',Xz,Yz
allocate(zetamean(Xz,Yz,TIME))
statusi = nf90_inq_varid(ncid2,'zeta_detided',zetaVarId2)
if (statusi /= nf90_noerr) statusi = nf90_inq_varid(ncid2,'zeta',zetaVarId2)
statusi = nf90_get_var(ncid2,zetaVarId2,zetamean)



do k=1,TIME
   zetameanval(k)=sum(zetamean(:,:,k), zetamean(:,:,k)<100)/(max(1,count(zetamean(:,:,k)<100)))
   write(*,*) "mean value zeta domainwide at t=",k,": ", zetameanval(k)
   !if (Xz < X) zeta(2:(X-1),2:(Y-1),k)=zeta(2:(X-1),2:(Y-1),k)-zetameanval
   !if (Xz == X) zeta(:,:,k)=zeta(:,:,k)-zetameanval
   zeta_detide(:,:,k)=zeta_detide(:,:,k)-zetameanval(k)
   zeta_tide(:,:,k)=zeta_tide(:,:,k)-zetameanval(k)
end do

!where (zeta > 1000.) zeta=1e+37

statuso = nf90_put_var(ncid,zetaDTVarId,zeta_detide)
statuso = nf90_put_var(ncid,zetaTVarId,zeta_tide)
statuso = nf90_close(ncid)



end program fastmeanzeta
